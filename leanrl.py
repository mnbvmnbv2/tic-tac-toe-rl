...
if __name__ == "__main__":
    ...
    def update(data):
        with torch.no_grad():
            ...
            td_target = (
                ...
                + args.gamma * target_max * (~data["dones"].flatten()).float()
            )
        old_val = (
            q_network(data["observations"])
            .gather(1, data["actions"].unsqueeze(-1))
            .squeeze()
        )
    ...
    eps_schedule = linear_schedule(
        args.start_e, args.end_e, args.exploration_fraction * args.total_timesteps
    )
    ...
    for global_step in pbar:
        ...
        next_obs = torch.as_tensor(next_obs, dtype=torch.float).to(
            device, non_blocking=True
        )
        terminations = torch.as_tensor(terminations, dtype=torch.bool).to(
            device, non_blocking=True
        )
        rewards = torch.as_tensor(rewards, dtype=torch.float).to(
            device, non_blocking=True
        )
